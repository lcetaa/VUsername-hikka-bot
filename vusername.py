...
class VUsernameMod(loader.Module):
    ...
    @loader.loop(UPDATE_CHECK_INTERVAL,autostart=True)
    async def _auto_update_loop(self):
        if not self.config.get("auto_update",True):return
        async with self._update_lock:
            try:
                differs,remote_ok=await self._check_update_hashes()
                if not remote_ok or not differs:return
                logger.info("Автообновление VUsername: найдена новая версия, устанавливаю...")
                ok=await self._safe_install_update()
                if ok:logger.info("Автообновление VUsername: успешно обновлено.")
                else:logger.warning("Автообновление VUsername: установка не удалась.")
            except Exception as e:
                logger.warning("Автообновление VUsername: ошибка фоновой проверки — %s",e)
